...
/* 
<applet code="Draw5" width=600 height=300>
</applet>
*/

public class Draw5 extends Applet implements ActionListener, MouseListener {
...
// Setting up data in Vectors so that it can be used to paint **********************************
  ...
  public void AddAll(Vector From, Vector Into) {
	for (int count=0; count < From.size(); count++) {
	  Into.addElement(From.elementAt(count));
	}
  }
...
// MouseListener Event Handling *********************************
  ...
  public void mouseClicked(MouseEvent e) {
	...
	if (mode.equals("CreatePolygon") && !clicks.isEmpty() && e.getX() > firstPointX-2 && e.getX() < firstPointX+2 && e.getY() > firstPointY-2 && e.getY() < firstPointY+2) {
	  ...
	  if (clicks.size() == 4) {
		AddAll(clicks, circles);
		circles.addElement(NewIndex);
		clicks.removeAllElements();
	  }
	}
